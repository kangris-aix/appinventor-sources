...
public class GDPRUtils {
    ...
    // Don't use below, it'll require permission setting
    /**
    ...
    public static Boolean isRequestInEAAOrUnknownViaTelephonyManagerCheck(Context context) {
        ...
        try {
            ...
            error = true;
            Log.d(LOG_TAG, "Could not get location from telephony manager via SimCountry", e);
        }
        ...
        try {
            ...
            error = true;
            Log.d(LOG_TAG, "Could not load location from network via NetworkCountry", e);
        }
...
//        String lastSep = context.getString(R.string.gdpr_last_list_seperator);
        ...
        return types.substring(types.length() - 1);
        /*int i = 0;
